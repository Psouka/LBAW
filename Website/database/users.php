<?php
    ...
    function assignSessionAttr($username)
    {
        ...
        $row = $stmt->fetch();
        if ($row) $_SESSION['usertype'] = 'admin';
        else $_SESSION['usertype'] = 'normal';
    }
    ...
    function createImagemUtilizador($file, $name)
    {
        ...
        $result = $stmt->fetch();
        if ($result) return $result['idimagemutilizador'];
        $stmt = $conn->prepare("
    ...
    function editProfile()
    {
        ...
        if (isset($_POST['first-name']) && $_POST['first-name'] != "") $newFirstName = $_POST['first-name'];
        else $newFirstName = $oldData['nomeproprio'];
        if (isset($_POST['last-name']) && $_POST['last-name'] != "") $newLastName = $_POST['last-name'];
        else $newLastName = $oldData['sobrenome'];
        if (isset($_POST['optionsRadios']) && $_POST['optionsRadios'] != "") $newGender = $_POST['optionsRadios'];
        else $newGender = $oldData['genero'];
        if (isset($_POST['birthDate']) && $_POST['birthDate'] != "") $newBirthDate = $_POST['birthDate'];
        else $newBirthDate = $oldData['datanascimento'];
        if (isset($_POST['email']) && $_POST['email'] != "") $newEmail = $_POST['email'];
        else $newEmail = $oldData['email'];
        if (isset($_POST['phoneNumber']) && $_POST['phoneNumber'] != "") $newPhone = $_POST['phoneNumber'];
        else $newPhone = $oldData['telefone'];
        if (isset($_POST['descricao']) && $_POST['descricao'] != "") $newDescricao = $_POST['descricao'];
        else $newDescricao = $oldData['descricao'];
        if (isset($_POST['password']) && isset($_POST['confirmpassword']))
        {
            ...
            else $newPassword = $oldData['palavrapasse'];
        }
        else $newPassword = $oldData['palavrapasse'];
        ...
        if ($oldData['idimagemcapa'] == "") $newCover = 2;
        else $newCover = $oldData['idimagemcapa'];
        global $conn;
        $stmt = $conn->prepare("
        UPDATE utilizador
    ...
    function searchUsers($searchtext)
    {
        ...
        ("
            ...
            WHERE (
                utilizador.utilizador @@to_tsquery(?) OR
